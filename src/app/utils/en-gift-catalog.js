...
// eslint-enable
export class enGiftCatalog {
  customDonationAmount = localStorage.getItem("customDonation")
  ...
  init() {
    console.log("enGiftCatalog init");
    this.onValidate();
    this.inputDonation = document.querySelector(
      "*[name='transaction.donationAmt.other']"
    );

    if (this.inputDonation) {
      this.inputDonation.value = 0;
      const donationButtons = document.querySelector(".en__field--donationAmt");
      // Hide Donation Amounts Radios
      donationButtons.style.display = "none";
      // Remove Recurrency Block
      const recurrencyBlock = document.querySelector(
        ".petaDonField_hideRecurring"
      );
      recurrencyBlock.remove();
  ...
  shouldRun() {
    // Only run if there's a donation form
  ...
  onValidate() {
    window.enOnValidate = function () {
      ...
      const repeatGift = document.querySelector(
        "*[name='transaction.recurrpay']"
      );
  ...
  getCustomDonationAmount() {
    ...
    if (parseFloat(customAmount.value) > 0) {
      this.updateInput(this.customDonationAmount);
    }
  ...
  createSelectAmount(slider, index) {
    ...
    this.values.push({ value: donationAmount, amount: currentAmount });
    ...
    decrease.addEventListener("click", function (e) {
      // const valueIndex = Math.max(this.values[index] - 1, 0);
      // this.values[index] = Math.max(this.values[index] - donationAmount, 0);
      const newAmount = Math.max(thisClass.values[index].amount - 1, 0);
      thisClass.values[index].amount = newAmount;
      amount.innerHTML = newAmount;
      thisClass.updateInput(thisClass.customDonationAmount);
    });

    increase.addEventListener("click", function (e) {
      // const valueIndex = Math.min(this.values[index] + 1, steps.length - 1);
      // this.values[index] = this.values[index] + donationAmount;
      const newAmount = thisClass.values[index].amount + 1;
      thisClass.values[index].amount = newAmount;
      amount.innerHTML = newAmount;
      thisClass.updateInput(thisClass.customDonationAmount);
    });
  ...
  listenMonthlyCheckbox() {
    ...
    const frequencyTitle = document.querySelector(".frequency-title");
    checkbox.addEventListener("change", function () {
      if (this.checked) {
        frequencyTitle.classList.remove("single");
        frequencyTitle.classList.add("monthly");
      } else {
        frequencyTitle.classList.remove("monthly");
        frequencyTitle.classList.add("single");
      }
    });
  }
  updateInput(customAmount) {
    const carePackagesTotal = document.querySelectorAll(".totalAmount");

    const result = this.values.reduce((acc, current) => {
    ...
    this.inputDonation.value = customAmount + result;
  }
  setSelectAmounts() {
    this.selectAmounts.forEach((slider, index) => {
      // this.values.push[0];
      this.createSelectAmount(slider, index);
    });
  }
  displayDonations() {
    let donationsHTML = "";
    ...
    const boxMessage = options.boxMessage || "Hilfspaket-Gesamtbetrag";
    ...
    options.donations.forEach((e, i) => {
      ...
      <div class="en__component--wrap">
        ...
        <div class="en__component en__component--content">
          ...
          <div class="en__component--quantity">Anzahl</div>
    ...
    shoppingCart.insertAdjacentHTML(
      "beforebegin",
      `
    <section id="FourSiteGiftCatalog">
    <input type="hidden" class="en__field__input en__field__input--hidden" name="transaction.recurrfreq" value="MONTHLY">
      <div class="en__component--hero-wrapper">
        <div
          class="en__component en__component--row en__component--row--1 en__component--hero"
          style="
            background: #418fde url('${options.heroImage}');
        ...
        >
          <div class="en__component--hero-wrap-responsive">
            ...
            <div class="en__component--hero-content">${options.heroCopy}</div>
          </div>

          <div class="en__component--hero-wrap">
            <div class="en__component--hero-logo"></div>
            <div class="en__component--hero-content">
              ${options.heroTitle} ${options.heroCopy}
            </div>
          </div>
        </div>
      </div>
      <div
        id="en__component--heading"
        class="en__component en__component--row en__component--row--3 en__component--heading"
      >
      ...
      <div class="en__component en__component--row en__component--row--4">
        <div
          class="en__component en__component en__component--column en__component--column--1"
        >
          <div
            class="en__field en__field--checkbox en__field--445200 en__field--recurrpay en__component--carePackage-content"
          >
            ...
            <div
              class="en__field__element en__field__element--checkbox carePackage-content"
            >
              <div class="en__field__item container">
                <input
                  id="en__field_transaction_recurrpay"
                  type="checkbox"
      ...
      <div
        class="en__component en__component--row en__component--row--5 en__component--complete-heading"
      >
        <div
          class="en__component en__component en__component--column en__component--column--1"
        >
    ...
    this.selectAmounts = document.querySelectorAll(".donationCard");

    if (this.selectAmounts) {
      this.getCustomDonationAmount();
      this.setSelectAmounts();
    }
  }
}
